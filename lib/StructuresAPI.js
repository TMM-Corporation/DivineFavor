...
var StructuresAPI = {
	...
	set: function (name, x, y, z, rotate, destroy, progressively, time, dropItems) {
		...
		new java.lang.Thread(function () {
			for (var i = 0; i < arr.length; i++) {
				...
				if (!destroy) World.setBlock(x + dx, y + dy, z + dz, id, data)
					...
					if (dropItems) dropList[i] = {id: id, data: data}
				}
				if (progressively) java.lang.Thread.sleep(time || 100)
				if (i == arr.length && dropItems)
					for (let u in dropList)
						World.drop(x, y, z, dropList[u].id, 1, dropList[u].data)
			}
		}).start()
	}
}
