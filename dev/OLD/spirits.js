...
var spirit = {
	...
	// mediums: ["medium_gold","medium_lapis","medium_iron","medium_gold","medium_gold","medium_obsidian","medium_redstone","medium_iron","medium_gold","medium_log"],
	// spirits: {
	...
	// 		{
	// 			// alert(i+"  "+spirit.callingStone[i]);
	// 			return i || null;
	// 		}
	// },
	// informate(id) {
	// 	if (spirit.getActive(id) != false) Game.message(t("Spirit: ") + spirit.list[id] + t(" is active"));
	// 	if (spirit.getActive(id) != true) Game.message(t("Spirit: ") + spirit.list[id] + t(" is inactive"));
	...
	trading_list: {},
	// getIdData(id){
	// 	if(Array.isArray(id)) return {id: id[0], data: id[1], count:1};
	// 	else return {id: id, data: 0, count: 1};
	// },
	...
	// tradeCheck(slots, spiritID){
	// 	for(let id in medium_Prototype.list){
	// 		let counter = 0, recipe = medium_Prototype.list[id][0];
	// 		recipeL: for(let i in recipe){
	// 			let item = this.getIdData(recipe[i]);
	// 			alert(slots[counter].id)
	// 			if(!this.checkIdData(item, slots[counter])) break recipeL;
	// 			counter++;
	// 			if(counter == recipe.length && talismans.talisman[id].spiritID == spiritID){
